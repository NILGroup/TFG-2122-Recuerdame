...
function writeTest($pdf, $informeSeguimiento){
    ...
    if(!empty($informeSeguimiento->getNombreEscala())){
        $pdf->Cell(50,7,utf8_decode($informeSeguimiento->getNombreEscala()),1);
...
function pdfBody($pdf, $informeSeguimiento, $paciente){
    ...
    $pdf->Ln();
    $pdf->SetFont('Times','',12);
    $pdf->MultiCell(0,7,utf8_decode($informeSeguimiento->getDiagnostico()),1);
    $pdf->Ln();
    ...
    if(!empty($informeSeguimiento->getObservaciones())){
        ...
        $pdf->Ln();
        $pdf->SetFont('Times','',12);
        $pdf->MultiCell(0,7,utf8_decode($informeSeguimiento->getObservaciones()),1);
        $pdf->Ln();
    }
...
if (!empty($_GET['idInforme'])){
    ...
    pdfBody($pdf,$informeSeguimiento, $paciente);
