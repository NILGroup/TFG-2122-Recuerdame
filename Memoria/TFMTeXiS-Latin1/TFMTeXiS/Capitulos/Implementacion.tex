...
\begin{figure}[!ht]
  	\centering
  	\includegraphics[width=12cm]{./Imagenes/Figuras/implementacion/login.png}
  	\caption{Pagina de login.}
	\label{fig:login}
\end{figure}

A continuación, una vez logueado como terapeuta nos aparecerá la página donde se muestran todos los pacientes del terapeuta, que esta compuesta por la cabecera de la aplicación que se verá en todas las pantallas de nuestra aplicación,así como la barra de navegación y el pie de página; con el logo de la aplicación en la parte superior izquierda, y en la otra esquina los iconos de cerrar, mensajería y otros pacientes. Luego nos encontramos la barra de navegación que nos permitirá acceder por todas las secciones (sesiones, evaluaciones, historias de vida y calendario) de la aplicación que comentaremos a continuación, con la foto e información del paciente que se seleccione de la lista; después el pie de página que constará del sello de copyright. En esta página se nos mostrará todos los pacientes que tenga un terapeuta asignado( Figura \ref{fig:listPac}).
