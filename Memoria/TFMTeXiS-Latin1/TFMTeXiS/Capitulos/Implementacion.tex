...
Para este TFG se ha desarrollado una aplicación web que será adaptable a dispositivo tablet y que actualmente se encuentra alojada en el servidor: \footnote{http://melpomene.fdi.ucm.es/recuerdame/}. 
En este capitulo mostraremos la arquitectura que se ha decidido para la aplicación así como el diseño de la base de datos implementado. Además se mostrarán cuales fueron las herramientas utilizadas y como fue implementada la aplicación.
...
\begin{itemize}
	...
	\item Como herramienta de control de versiones hemos utilizado GitHub debido a la facilidad de uso ya que podemos ver en todo momento los cambios realizados por nuestros compañeros.
\end{itemize}
...
La estructura de la base de datos (Figura \ref{fig:str_bd}) fue definida, teniendo en cuenta toda la información aportada por los terapeutas durante las entrevistas y durante la evaluación del diseño competitivo. En las siguientes subsecciones se explica en detalle las características de cada tabla.
\begin{figure}[!ht]
	\centering
...
Contiene toda la información de los usuarios (terapeutas y cuidadores) que pueden iniciar sesión en la aplicación. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_usuario}: El id del usuario.
	\item \texttt{nombre\_usuario}: El nombre del usuario.
	\item correo: El correo del usuario.
	\item contrasenia: La contraseña asociada al correo del usuario.
	\item nombre: El nombre del usuario.
	\item apellidos: Los apellidos del usuario.
	\item rol: El rol del usuario (terapeuta o cuidador).
	
\end{itemize}

\subsection{Tabla paciente}
Tabla con la información de los pacientes a tratar que podrán ser asignados a los terapeutas o a un cuidador. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_paciente}: El id del paciente.
	\item nombre: El nombre del paciente.
	\item apellidos: Los apellidos del paciente.
	\item genero: El genero del paciente.
	\item \texttt{lugar\_nacimiento}: El lugar de nacimiento del paciente.
	\item nacionalidad: La nacionalidad del paciente.
	\item \texttt{fecha\_nacimiento}: La fecha de nacimiento del paciente.
	\item \texttt{tipo\_residencia}: El tipo de residencia del paciente (centro de día, residencia de ancianos, su casa...).
	\item \texttt{residencia\_actual}: Dirección donde se encuentra actualmente el paciente.
	\item \texttt{id\_cuidador}: El id del cuidador asociado a ese paciente.
	
\end{itemize}

\subsection{Tabla sesion}
Tabla con la información de las sesiones que el terapeuta realiza con los pacientes. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_sesion}: El id de la sesión.
	\item fecha: La fecha en la que se realiza la sesión.
	\item \texttt{id\_etapa}: La etapa asociada al recuerdo de esa sesión.
	\item objetivo: El objetivo de la sesión.
	\item descripcion: La descripción de la sesión.
	\item barreras: Si el paciente se opone a hablar de algunos temas.
	\item facilitadores: Los temas en los que el paciente se siente más cómodo.
	\item \texttt{fecha\_finalizada}: La fecha de finalización de la sesión.
	\item \texttt{id\_paciente}: El id del paciente.
	\item \texttt{id\_usuario}: El id del usuario.
	\item respuestas: Las respuestas que se obtienen de esa sesión.
	\item observaciones: Las observaciones que se obtienen de la sesión.
	
\end{itemize}

\subsection{Tabla recuerdo}
Tabla con la información de los recuerdos de todos los pacientes. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_recuerdo}: El id del recuerdo.
	\item fecha: La fecha aproximada en la que se da ese recuerdo.
	\item nombre: El nombre que se le da a ese recuerdo.
	\item descripcion: Una descripción breve del recuerdo.
	\item localizacion: El lugar donde se da el recuerdo.
	\item \texttt{id\_etapa}: La etapa de vida en la que se encuentra el paciente durante dicho recuerdo.
	\item \texttt{id\_categoría}: La categoría del recuerdo.
	\item \texttt{id\_emocion}: La emoción que le provoca ese recuerdo al paciente.
	\item \texttt{id\_estado}: El estado del recuerdo, si esta conservado o en riesgo de perder.
	\item \texttt{id\_etiqueta}: Si es un recuerdo positivo, negativo o neutro.
	\item puntuacion: Una puntuación siendo 0 de menos relevante a 10 más relevante.
	\item \texttt{id\_paciente}: El id del paciente

\end{itemize}

\subsection{Tabla multimedia}
Tabla con la información de los acrhivos multimedia de todos los pacientes. Estos archivos multimedia se utilizan tanto en las sesiones como en los recuerdos. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_multimedia}: El id de la multimedia.
	\item nombre: El nombre del fichero.
	\item fichero: El fichero multimedia.

\end{itemize}

\subsection{Tabla evaluacion}
Tabla con la información de las evaluaciones que hacen los terapeutas a los pacientes para ver su progreso. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_evaluacion}: El id de la evaluación.
	\item \texttt{id\_paciente}: El id del paciente.
	\item fecha: La fecha en la que se lleva a cabo la evaluación.
	\item gds: El grado de GDS.
	\item \texttt{gds\_fecha}: La fecha en la que se le pone ese grado de GDS.
	\item mental: El grado de mini-mental.
	\item \texttt{mental\_fecha}: La fecha en la que se le pone ese nivel.
	\item cdr: El grado de CDR.
	\item \texttt{cdr\_fecha}: La fecha en la que se pone ese grado.
	\item diagnostico: El diagnóstico que considera el terapeuta si ha evolucionado o no.
	\item observaciones: Las observaciones del terapeuta.
	\item \texttt{nombre\_escala}: Otro tipo de escala que los terapeutas consideren necesaria.
	\item escala: El grado de escala.
	\item \texttt{fecha\_escala}: La fecha en la que se le da ese grado.
	
\end{itemize}

\subsection{Tabla \texttt{persona\char`_relacionada}}
Tabla con la información de las personas que están relacionadas con los pacientes. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_persona\_relacionada}: El id de la persona relacionada.
	\item nombre: El nombre de la persona relacionada.
	\item apellidos: Los apellidos de la persona relacionada.
	\item telefono: El teléfono de la persona relacionada.
	\item ocupacion: La ocupación actual de la persona relacionada.
	\item email: El email de la persona relacionada.
	\item \texttt{id\_tipo\_relacion}: El tipo de relación que tiene con el paciente.
	\item \texttt{id\_paciente}: El id del paciente asociado.

\end{itemize}

\subsection{Tabla actividad}
Tabla con la información de las actividades que el terapeuta ha asignado al paciente para que las realice con su cuidador. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item id: Id de la actividad.
	\item start: Cuando da comienzo la actividad.
	\item title: El título de la actividad.
	\item description: La descripción de la actividad.
	\item \texttt{id\_paciente}: El id del paciente.
	\item color: El color que se le pone a la actividad.

\end{itemize}

\subsection{Tabla \texttt{tipo\char`_relacion}}
Tabla que contiene los tipos de relación que pueden tener las personas que están relacionadas con los pacientes. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_tipo\_relacion}: El tipo de relación con el paciente.
	\item nombre: El nombre de la persona.

\end{itemize}

\subsection{Tabla etiqueta}
Tabla que contiene las etiquetas que pueden tener los recuerdos de los pacientes y que sirven para indicar si un recuerdo es positivo, neutro o negativo. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_etiqueta}: Id de la etiqueta.
	\item nombre: El nombre de la etiqueta.

\end{itemize}

\subsection{Tabla etapa}
Tabla que contiene las etapas de vida que pueden tener las sesiones y los recuerdos de los pacientes. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_etapa}: Id de la etapa.
	\item nombre: El nombre de la etapa.

\end{itemize}

\subsection{Tabla estado}
Tabla que contiene las tipo de estados que pueden tener los recuerdos de los pacientes y que sirven para indicar si un recuerdo está conservado, en riesgo de perder o perdido. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_estado}: Id de estado.
	\item nombre: El nombre del estado.

\end{itemize}

\subsection{Tabla emocion}
Tabla que contiene las tipo de emoción que pueden tener los recuerdos de los pacientes y que sirven para indicar la emoción que le surge al paciente con el recuerdo. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_emocion}: Id de la emoción.
	\item nombre: El nombre de la emoción.

\end{itemize}

\subsection{Tabla categoria}
Tabla que contiene las tipo de categorías que pueden tener los recuerdos de los pacientes y que sirven para indicar con qué está relacionado el recuerdo (familia, hobbies, amistad, trabajo, política, estudios u otros). Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_categoria}: Id de la categoría.
	\item nombre: El nombre de la categoría.

\end{itemize}

\subsection{Tabla terapeuta}
Tabla con la relación de terapeutas y pacientes que sirve para asignar los pacientes a varios terapeutas. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_paciente}: Id del paciente.
	\item \texttt{id\_terapeuta}: Id del terapeuta.

\end{itemize}

\subsection{Tabla \texttt{sesion\char`_recuerdo}}
Tabla con la relación de sesiones y recuerdos que sirve para asignar los recuerdos de los pacientes a las sesiones. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_sesion}: Id de la sesión.
	\item \texttt{id\_recuerdo}: Id del recuerdo.

\end{itemize}

\subsection{Tabla \texttt{sesion\char`_multimedia}}
Tabla con la relación de sesiones y archivos multimedia que sirve para asignar los archivos multimedia a las sesiones de los pacientes. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_sesion}: Id de la sesión.
	\item \texttt{id\_multimedia}: Id de la multimedia.

\end{itemize}

\subsection{Tabla \texttt{recuerdo\char`_persona\char`_ralacionada}}
Tabla con la relación de recuerdos y personas relacionadas de los pacientes que sirve para asignar las personas relacionadas a los recuerdos de los pacientes. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_recuerdo}: Id del recuerdo.
	\item \texttt{id\_persona\_relacionada}: Id de la persona relacionada.

\end{itemize}

\subsection{Tabla \texttt{recuerdo\char`_multimedia}}
Tabla con la relación de recuerdos y archivos multimedia que sirve para asignar los archivos multimedia a los recuerdos de los pacientes. Los atributos se pueden ver a continuación:
\begin{itemize}
	\item \texttt{id\_recuerdo}: Id del recuerdo.
	\item \texttt{id\_multimedia}: Id de la multimedia.
...
\begin{figure}[!ht]
  	\centering
  	\includegraphics[width=8cm]{./Imagenes/Figuras/implementacion/inicio.png}
...
Para generar el libro de Historias de Vida el usuario indica los filtros que desee (fecha, etiqueta y etapa de vida)  y hace una petición mediante el botón ``Generar Historia de Vida'' y el front envía una petición al gestor de las vistas del front que a su vez llama al Controlador (ControladorHistoriaVida) que mediante la función generarLibroHistoriaVida  hace una petición al modelo Recuerdo para que recupere la información de la base de datos y devuelva un listado de recuerdos.
...
A continuación se muestra el PDF de Historia de Vida generado (ver Figura \ref{fig:pdf.png}). 
\begin{figure}[!ht]
  	\centering
